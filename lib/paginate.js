...
export default async function fetchRootPaginatedFields(query, cursorVars, fetchGraphQL) {
  ...
  while (Object.values(finished).some(v => !v)) {
    ...
    if (res.errors) {
      throw new Error(`GraphQL error during pagination: ${res.errors.map(e => e.message).join(', ')}`)
    }
    ...
    for (const path in filteredVars) {
      ...
      if (!section?.nodes) {
        logger.warn(`No nodes found for paginated field "${path}", marking as finished`)
        finished[path] = true
        continue
      }
...
// Named export as well as the default, so `import { fetchRootPaginatedFields } from …` works.
export { fetchRootPaginatedFields }
