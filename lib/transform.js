...
const logger = pino(
  process.env.FETCH_LOG_LEVEL
    ? {
        name: 'transformPaginationArgs',
        level: process.env.FETCH_LOG_LEVEL,
        transport: { target: 'pino-pretty', options: { colorize: true, singleLine: true } }
      }
    : { enabled: false }  // Disable logging if FETCH_LOG_LEVEL is not set
)
...
export default function transformPaginationArgs(query, autoKeyword = 'auto') {
  ...
  const transformed = visit(ast, {
    Field(node) {
      // Safely handle node.arguments being undefined
      ...
      if (firstArgIndex !== -1) {
        ...
        // Inject pageInfo { hasNextPage endCursor } if not present
        let selectionSet = node.selectionSet
        if (selectionSet && !selectionSet.selections.some(sel =>
          sel.kind === 'Field' && sel.name.value === 'pageInfo'
        )) {
          selectionSet = {
            ...selectionSet,
            selections: [
        ...
        return { ...node, arguments: newArgs, selectionSet }
      }
    },
    OperationDefinition: {
      enter(node) {
...
// Named export as well as the default, so `import { transformPaginationArgs } from …` works.
export { transformPaginationArgs }
